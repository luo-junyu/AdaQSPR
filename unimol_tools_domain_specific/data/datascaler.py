...
class TargetScaler(object):
    ...
    def fit(self, target, dump_dir):
        """
        Fits the scaler to the target values and optionally saves the scaler to disk.生成'target_scaler.ss' 文件

        当任务类型是 'regression' 时，通过 is_skewed 方法判断 target 是否有偏斜，如果有偏斜则选择 'robust' 缩放器（SCALER_MODE['robust']()），否则选择 'standard' 缩放器（SCALER_MODE['standard']()），然后对选择的缩放器进行拟合操作（self.scaler.fit(target)）。
        当任务类型是 'multilabel_regression' 时，先创建一个空列表 self.scaler，屏蔽 target 中的无效值（NaN）；然后对 target 的每一列，通过 is_skewed 方法判断是否有偏斜，根据情况选择 'robust' 或 'standard' 缩放器添加到 self.scaler 列表中，并对每个缩放器进行拟合操作（self.scaler[-1].fit(target[:, i:i+1])）。
    ...
    def inverse_transform(self, target):
        """
        Inverse transforms the scaled target values back to their original scale. inverse_transform 方法用于将已缩放的目标值逆变换回原始尺度。
    ...
    def is_skewed(self, target):
        """
        Determines whether the target values are skewed based on skewness and kurtosis metrics.is_skewed 方法用于根据偏度和峰度指标判断目标值是否有偏斜
