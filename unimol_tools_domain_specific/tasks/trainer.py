...
class Trainer(object):
    ...
    def decorate_torch_batch(self, batch):
        ...
        :return: A tuple of (net_input, net_target) for model processing.
        """
        # net_input, net_mask, net_target = batch
        ...
        # net_mask = net_mask.to(self.device)
        # return net_input, net_mask, net_target
    ...
    def batch_collate_fn(self, samples):
        ...
        for k in samples[0][0].keys():
            if k == 'src_coord':
                ...
                v = pad_2d([torch.tensor(s[0][k]).long() for s in samples], pad_idx=self.model.padding_idx)
            elif k == 'src_distance':
                v = pad_2d([torch.tensor(s[0][k]).float() for s in samples], pad_idx=0.0)
            elif k == 'src_tokens':
                v = pad_1d_tokens([torch.tensor(s[0][k]).long() for s in samples], pad_idx=self.model.padding_idx)
            batch[k] = v

        mask = torch.tensor([s[1] for s in samples])
        label = torch.tensor([s[-1] for s in samples])
        return batch, mask, label
