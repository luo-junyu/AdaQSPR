...
class MolTrain2(object):
    ...
    def fit(self, data):
        """
            ...
            data2: list of data
        """
        self.datahub_list = [DataHub(data = d, is_train=True, save_path=self.save_path, **self.config) for d in data]
        self.data = [dh.data for dh in self.datahub_list]

        self.update_and_save_config()
        self.trainer = Trainer2(save_path=self.save_path, **self.config)
        self.model = NNModel2(self.data, self.trainer, **self.config)
        self.model.run()
        scalar = self.data[0]['target_scaler']
        y_pred = self.model.cv['pred']
        y_true = np.array(self.data[0]['target'])
    ...
    def update_and_save_config(self):
        ...
        """
        self.config['num_classes'] = self.data[0]['num_classes']
        self.config['target_cols'] = ','.join(self.data[0]['target_cols'])
        if self.config['task'] == 'multiclass':
            self.config['multiclass_cnt'] = self.data[0]['multiclass_cnt']
