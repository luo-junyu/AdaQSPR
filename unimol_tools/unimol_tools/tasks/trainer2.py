...
def masked_average_pooling(feats, masks):
    masked_feats = [feat * mask.unsqueeze(-1) for feat, mask in zip(feats, masks)]
    sum_feats = torch.sum(torch.stack(masked_feats), dim=0)
    sum_masks = torch.sum(torch.stack(masks), dim=0).unsqueeze(-1)
    pooled_feats = sum_feats / sum_masks
    return pooled_feats 
...
class Trainer2(object):
    ...
    def decorate_torch_batch(self, batch):
        ...
        net_input = [{k: v.to(self.device) for k, v in nInput.items()} for nInput in net_input]
        # net_input_2 = {k: v.to(self.device) for k, v in net_input_2.items()}
    ...
    def batch_collate_fn2(self, samples):
        ...
        for idx in range(8):
            for k in samples[0][0][0].keys():
                if k == 'src_coord':
                    v = pad_coords([torch.tensor(s[0][idx][k]).float() for s in samples], pad_idx=0.0)
                elif k == 'src_edge_type':
                    v = pad_2d([torch.tensor(s[0][idx][k]).long() for s in samples], pad_idx=self.model.padding_idx)
                elif k == 'src_distance':
                    v = pad_2d([torch.tensor(s[0][idx][k]).float() for s in samples], pad_idx=0.0)
                elif k == 'src_tokens':
                    v = pad_1d_tokens([torch.tensor(s[0][idx][k]).long() for s in samples], pad_idx=self.model.padding_idx)
                batch[idx][k] = v
            batch[idx]['mask'] = torch.tensor([s[1][idx] for s in samples]).long()
        # try:
    ...
    def fit_predict(self, model, train_dataset, valid_dataset, loss_func, activation_fn, dump_dir, fold, target_scaler, feature_name=None):
        ...
        train_dataloader = NNDataLoader(
            ...
            shuffle=True,
            collate_fn=self.batch_collate_fn2,
            # drop_last=True,
        )
        ...
        for epoch in range(self.max_epochs):
            ...
            for i, batch in enumerate(train_dataloader):
                net_inputs, net_target = self.decorate_batch(batch, feature_name)
                ...
                with torch.set_grad_enabled(True):
                    ...
                    for i, nInput in enumerate(net_inputs):
                        net_inputs[i]['return_repr'] = True
                        out_feat = model(**nInput)
                        out_feat = out_feat['cls_repr']
                        if i < 4:
                            repr_1.append(out_feat)
                            masks_1.append(nInput['mask'][:,0])
                            feats_1.append(nInput['mask'][:,1:])
                        else:
                            repr_2.append(out_feat)
                            masks_2.append(nInput['mask'][:,0])
                            feats_2.append(nInput['mask'][:,1:])
                    
                    all_repr_1 = [model.embedding_head(torch.cat([r,f], axis=1)) for r,f in zip(repr_1, feats_1)]
                    all_repr_2 = [model.embedding_head(torch.cat([r,f], axis=1)) for r,f in zip(repr_2, feats_2)]

                    pooled_repr_1 = masked_average_pooling(all_repr_1, masks_1)
                    pooled_repr_2 = masked_average_pooling(all_repr_2, masks_2)
                    
                    # pooled_repr_1 = repr_1[0]
                    # pooled_repr_2 = repr_2[0]
                    # global_feat, mol_feat, jituan_feat, qiangji_feat
                    # feats_1 = feats_1[0]
                    # feats_2 = feats_2[0]

                    out_feat = torch.cat([pooled_repr_1, pooled_repr_2], dim=1)
                    # out_feat = torch.cat([pooled_repr_1, feats_1[:,3:4], pooled_repr_2, feats_2[:,3:4]], dim=1)
                    # out_feat = torch.cat([pooled_repr_1, pooled_repr_2], dim=1)
                    logits = model.classification_head(out_feat)
                    loss = loss_func(logits, net_target)
    ...
    def predict(self, model, dataset, loss_func, activation_fn, dump_dir, fold, target_scaler=None, epoch=1, load_model=False, feature_name=None):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                # all_repr_1 = [torch.cat(r,f) for r,f in zip(repr_1, feats_1)]
                # all_repr_2 = [torch.cat(r,f) for r,f in zip(repr_2, feats_2)]
                ...
                # pooled_repr_1 = masked_average_pooling(repr_1, masks_1)
                # pooled_repr_2 = masked_average_pooling(repr_2, masks_2)
                ...
                # out_feat = torch.cat([pooled_repr_1, feats_1, pooled_repr_2, feats_2], dim=1)
                # out_feat = torch.cat([pooled_repr_1, feats_1[:,3:4], pooled_repr_2, feats_2[:,3:4]], dim=1)
                out_feat = torch.cat([pooled_repr_1, pooled_repr_2], dim=1)

                outputs = model.classification_head(out_feat)
