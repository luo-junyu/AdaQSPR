...
def main():
    # read from excel
    df = pd.read_excel('../data/candidate/Tg.xlsx', sheet_name='Sheet1')  #change the file name
    ...
    clf = MolPredict2(load_model=f'./exp/exp-poly-{cate}-our_train_result')
    predict = clf.predict(data)
    sc = joblib.load(f'./exp/exp-poly-{cate}-our_train_result/target_scaler.ss')
    p_inverse = sc.inverse_transform(predict)
    print(p_inverse.max())
    df[f'{tcol}_pred'] = np.array(list(p_inverse))
    df.to_excel(f'../data/pred/prediction_candidate_{cate}.xlsx', index=False)   #change the file name
    df.keys()
